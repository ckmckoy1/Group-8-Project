document.addEventListener('DOMContentLoaded', function () {
    ...
    function initializeDataTable() {
        table = $('#orderTable').DataTable({
            ...
            pageLength: 10,
            scrollX: true,
            scrollY: '50vh', // Corrected scrollY value
            scrollCollapse: true, // Allow the table to collapse its height when fewer records are present
            orderCellsTop: true,
            colReorder: {
            ...
            columnDefs: [
                ...
                { name: 'Billing Zip', targets: 12, orderable: true }, // Added missing columnDef
    ...
    function displayOrders(orders) {
        ...
        orders.forEach(order => {
            ...
            row.innerHTML = `
                <td>${order.OrderID || 'N/A'}</td>            <!-- index 0 -->
                <td>${order.FirstName || ''} ${order.LastName || ''}</td> <!-- index 1 -->
                <td>${order.CustomerEmail || 'N/A'}</td>       <!-- index 2 -->
                <td>${order.ShippingMethod || 'N/A'}</td>      <!-- index 3 -->
                <td>${order.ShippingAddress || 'N/A'}</td>     <!-- index 4 -->
                <td>${order.UnitNumber || 'N/A'}</td>          <!-- index 5 -->
                <td>${order.ShippingCity || 'N/A'}</td>        <!-- index 6 -->
                <td>${order.ShippingState || 'N/A'}</td>       <!-- index 7 -->
                <td>${order.ShippingZip || 'N/A'}</td>         <!-- index 8 -->
                <td>${order.BillingAddress || 'N/A'}</td>      <!-- index 9 -->
                <td>${order.BillingCity || 'N/A'}</td>         <!-- index 10 -->
                <td>${order.BillingState || 'N/A'}</td>        <!-- index 11 -->
                <td>${order.BillingZipCode || 'N/A'}</td>      <!-- index 12 -->
                <td>$${order.TotalAmount ? order.TotalAmount.toFixed(2) : '0.00'}</td> <!-- index 13 -->
                <td>${order.PaymentStatus || 'N/A'}</td>       <!-- index 14 -->
                <td>**** **** **** ${order.CardNumber ? order.CardNumber.slice(-4) : 'N/A'}</td> <!-- index 15 -->
                <td>${order.CardBrand || 'N/A'}</td>           <!-- index 16 -->
                <td>${order.ExpirationDate || 'N/A'}</td>      <!-- index 17 -->
                <td>${order.TransactionDateTime ? new Date(order.TransactionDateTime).toLocaleString() : 'N/A'}</td> <!-- index 18 -->
                <td>${order.OrderDate || 'N/A'}</td>           <!-- index 19 -->
                <td>${order.OrderTime || 'N/A'}</td>           <!-- index 20 -->
                <td>${order.AuthorizationToken || 'N/A'}</td>  <!-- index 21 -->
                <td>$${order.AuthorizationAmount ? order.AuthorizationAmount.toFixed(2) : '0.00'}</td> <!-- index 22 -->
                <td>${order.AuthorizationExpirationDate ? new Date(order.AuthorizationExpirationDate).toLocaleString() : 'N/A'}</td> <!-- index 23 -->
    ...
    function updateTotals() {
        ...
        $('#orderTable tbody tr').each(function () {
            const amountText = $(this).find('td').eq(13).text().replace('$', '');
            const amount = parseFloat(amountText) || 0;  // Updated index for Total Amount

            const transactionAmountText = $(this).find('td').eq(22).text().replace('$', '');
            const transactionAmount = parseFloat(transactionAmountText) || 0; // Updated index for Authorization Amount
    ...
    const downloadConfirmButton = document.getElementById('downloadConfirm');
    if (downloadConfirmButton) {
        ...
        console.error('Element with ID "downloadConfirm" not found.');
    }
    ...
    if (refreshTableButton) {
        ...
        console.error('Element with ID "refreshTable" not found.');
    }

    // Choose columns Button
    const chooseColumnsButton = document.getElementById('chooseColumns');
    if (chooseColumnsButton) {
        chooseColumnsButton.addEventListener('click', function () {
            populateColumnChooser(); // Populate the modal
        });
    } else {
        console.error('Element with ID "chooseColumns" not found.');
    }
    ...
    function populateColumnChooser() {
        ...
        if (!availableColumns || !selectedColumns) {
            console.error('Column chooser lists not found.');
            return;
        }
    ...
    function updateTableColumns() {
        ...
        selectedColumns.forEach(item => {
            ...
            if (!isNaN(columnIdx)) { // Ensure columnIdx is a valid number
                ...
                console.warn(`Invalid column index: ${columnIdx}`);
            }
        });
    ...
    if (applyColumnsButton) {
        ...
        console.error('Element with ID "applyColumns" not found.');
    }
});
